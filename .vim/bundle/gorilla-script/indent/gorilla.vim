...
setlocal autoindent
setlocal indentexpr=GetGorillaIndent(v:lnum)
" Make sure GetGorillaIndent is run when these are typed so they can be
" indented or outdented.
setlocal indentkeys+=0],0),0.,=else,=when,=catch,=finally

" Only define the function once.
if exists("*GetGorillaIndent")
  finish
endif
...
let s:OUTDENT_AFTER = '^\%(return\|returnif\|break\|continue\|throw\)\>'

" A compound assignment like `... = if ...`
let s:COMPOUND_ASSIGNMENT = '[:=]\s*\%(if\|unless\|for\|while\|until\|'
\                         . 'asyncif\|asyncunless\|asyncfor\|asyncwhile\|asyncuntil\|'
\                         . 'switch\|try\|class\)\>'
...
let s:SYNTAX_STRING = 'gorilla\%(String\|Regex\|Heregex\|'
\                   . 'HereString\)'

" Syntax names for comments
let s:SYNTAX_COMMENT = 'gorilla\%(Comment\|LineComment\|HeregexComment\)'
...
function! GetGorillaIndent(curlinenum)
